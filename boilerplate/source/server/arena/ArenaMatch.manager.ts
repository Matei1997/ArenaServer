...
import {
    ARENA_CONFIG, WEAPON_ROTATION, VEHICLE_POOL, ARENA_AMMO
} from "./ArenaConfig";
import { startZone, stopZone } from "./ZoneSystem";
import { initPlayerItems, clearPlayerItems, cancelCast } from "./ItemsSystem";
...
function getWeaponsForRound(round: number): number[] {
    const idx = (round - 1) % WEAPON_ROTATION.length;
    return WEAPON_ROTATION[idx].weapons;
}

function getWeaponRoundName(round: number): string {
    const idx = (round - 1) % WEAPON_ROTATION.length;
    return WEAPON_ROTATION[idx].name;
}
...
function giveRoundWeapons(player: PlayerMp, round: number): void {
...
function destroyMatchVehicles(match: ArenaMatchData): void {
    match.vehicles.forEach((veh) => {
        try { if (mp.vehicles.exists(veh)) veh.destroy(); } catch { /* ignore */ }
    });
    match.vehicles = [];
}
...
function spawnPlayerAtArena(player: PlayerMp, spawn: { x: number; y: number; z: number; heading?: number }, dimension: number): void {
    ...
    player.health = 100 + ARENA_CONFIG.startHealth;
    player.armour = ARENA_CONFIG.startArmor;
...
function beginRound(match: ArenaMatchData): void {
    ...
    [...match.redTeam, ...match.blueTeam].forEach((mp_) => initPlayerItems(mp_.id));
...
export function handleArenaDeath(victim: PlayerMp, killer: PlayerMp | undefined): boolean {
    ...
    victimData.deaths++;
    cancelCast(victim.id);
    clearPlayerItems(victim.id);
...
export function leaveMatch(player: PlayerMp, returnToMenu: boolean = true): boolean {
    ...
    cancelCast(player.id);
    clearPlayerItems(player.id);
    player.dimension = 0;
    player.removeAllWeapons();
...
export function endMatch(dimension: number): void {
    ...
    const winner: Team | "draw" =
        match.redScore > match.blueScore ? "red" : match.blueScore > match.redScore ? "blue" : "draw";
